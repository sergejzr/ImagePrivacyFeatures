...
public class FeatureReader {
	 ...
	 * @return
	 */
	public Map<String, Map<String, VisualFeature>> readFromDatabaseEntries(
			ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				Map<String, Map<String, VisualFeature>> fromdb = readFromDatabaseEntry(
						featuretype, entry);
	...
	public void combineFeatureSets(Map<String, Map<String, VisualFeature>> in,
			String featuretype, Map<String, Map<String, VisualFeature>> out) {
		for (String key : out.keySet()) {
			putAll(in, key, out.get(key));
		}
	}

	public void putAll(Map<String, Map<String, VisualFeature>> contiset,
			String featuretype, Map<String, VisualFeature> features) {
		Map<String, VisualFeature> conti = contiset.get(featuretype);
		if (conti == null) {
			contiset.put(featuretype,
					conti = new HashMap<String, VisualFeature>());
		}
		conti.putAll(features);
	}

	public Map<String, Map<String, VisualFeature>> readFromDatabaseEntry(
			String featuretype, String databaseentry) {
		Map<String, Map<String, VisualFeature>> ret = new Hashtable<String, Map<String, VisualFeature>>();
	...
	public Map<String, VisualFeatureGroup> readFeatures(String id,
	...
	public VisualFeatureGroup readFeatures(String id,String featurename,
			String featuredescriptiontext) {
		ftype type = featuretypes.get(featurename);
		//HashMap<String, VisualFeatureGroup> curFeatureContainer = new HashMap<String, VisualFeature>();
	...
	private static VisualFeatureGroup getSiftFeature(
			String id, String content, String featurename) {
		...
		if (lines.length > startidx) {
			for (int i = 2; i < lines.length; i++) {
				...
				if (f == null) {
					cntfeatures.put(featurename + "_" + key,
							f = new VisualFeature(featurename + "_" + key, 1.));
				}
		...
		VisualFeatureGroup ret=new VisualFeatureGroup(id,featurename,lines.length>1?lines[0]+"\n"+lines[0]:lines.length==1?lines[0]:"");
		ret.setFeatures(cntfeatures);
		return ret;
		//return new VisualFeatureGroup(lines.length>1?lines[0]+"\n"+lines[0]:lines.length==1?lines[0]:"", cntfeatures);
	}

	private static VisualFeatureGroup getFeature(
			String id,String content, String featurename, ftype type) {
		HashMap<String, VisualFeature> cntfeatures = new HashMap<String, VisualFeature>();
			...
			for (int i = 1; i < lines.length; i++) {
				...
				for (String featurevalue : featuresperline) {
					...
					if (numbertoadd != 0.) {
						...
						if (f == null) {
							cntfeatures.put(curfeaturename,
									f = new VisualFeature(curfeaturename,
											numbertoadd));
						}
						
						// should add here info about the feature. For example its coordinate in the picture
						f.addFeatureInfo("");
						/*
		...
		if (!featureexists) {
			...
			if (f == null) {
				cntfeatures.put(featurename + "_NO", f = new VisualFeature(
						featurename + "_NO", 1.));
			}
			f.addFeatureInfo("");
			/*
			 * Double cnt=cntfeatures.get(featurename+"_NO"); if(cnt==null) {
			 * cnt=0.; } cnt++; cntfeatures.put(featurename+"_NO", 1.);
			 */
		...
		return ret;
		 ...
		 //	return new VisualFeatureGroup(lines.length>0?lines[0]:"", cntfeatures);
