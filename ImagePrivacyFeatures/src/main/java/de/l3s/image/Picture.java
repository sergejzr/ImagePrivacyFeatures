...
public abstract class Picture {
	...
	/*
	 * public Picture(String id, String pictureTitle, String pictureTags,
				String pictureDescription, String imageURL, String filename) {
	...
	public  void storePicture(File directory) throws MalformedURLException, IOException
	{
		...
		if(newimpath!=null){
		storePictureFromBytes(newimpath,
				img);
		}
		}
	}
	...
	private void storeFeatureDescriptions(File directory,
	...
	/*
	protected void storeVisualFeatures(File directory,
			Map<String, VisualFeatureGroup> features) {
		...
		try {
			...
			for(String fname:features.keySet())
			{
				...
				Map<? extends String, ? extends VisualFeature> fgroups = feature.getFeatures();
	...
	*/
	protected void storeVisualFeaturesSQLFile(File directory,
			Map<String, VisualFeatureGroup> features) {
		
		try {
			
			Hashtable<String, String> sqlfiles = Features2SQLString
					.toSQLString(getStorageId(), features);
	...
	public Map<String, VisualFeatureGroup> getVisualFeatures(
			Set<String> usefeaturesidx, FeatureExtractor featureExtractor) throws IOException
			{
	...
	protected Map<String, VisualFeatureGroup> getVisualFeatures(File imagedir,
			Set<String> usefeaturesidx, FeatureExtractor featureExtractor) throws IOException {
		...
		ret = converter
		.convertToFeatures(id, featuredescriptions);



		if(ret.size()>0){ hasvisualfeatures = true;}
		...
		Map<String, VisualFeatureGroup> computedmissingfeatures = computeVisualFeatures(imagedir,missingfeatures, featureExtractor);
	...
	protected Map<String, VisualFeatureGroup> getVisualSQLFeatures(File imagedir,
			Set<String> usefeaturesidx, FeatureExtractor featureExtractor) throws IOException {
	...
	protected Map<String, VisualFeatureGroup> computeVisualFeatures(
			File imagedir, Set<String> usefeaturesidx, FeatureExtractor featureExtractor) throws IOException {
		...
		if (image!=null&&featureExtractor != null) {
			...
			try {
				try{
				featuredescriptions = featureExtractor.extractFrom(image,
						usefeaturesidx);
				}catch (Exception e) {
					e.printStackTrace();
					return tmpfeatures;
				...
				Map<String, VisualFeatureGroup> visualfeatures = converter
						.convertToFeatures(id, featuredescriptions);
	...
	public void setTextualMetadata(String pictureTitle, String pictureTags,
			String pictureDescription)
	{
			this.pictureTitle = pictureTitle;
			this.pictureTags = pictureTags;
...
*/
	...
	public static BufferedImage resizeImage(BufferedImage image, int maxWidth,
			int maxHeight) {
	...
	public void storePictureFromBytes(File tmppicfolder, byte[] data) {
		...
		try {
			BufferedImage img = ImageIO.read(new ByteInputStream(data, 0,
					data.length));
	...
	public Map<String, VisualFeature> featuresToList(HashSet<String> usefeaturesidx,FeatureExtractor featureExtractor) throws IOException {
	...
	public void computeVisualFeatures(HashSet<String> usefeaturesidx,
	...
	public void gatherVisualFeatures(HashSet<String> usefeaturesidx,
